...
def get_decoding_args(cwf="instruction", model_id="Meta-Llama-3.1-8B", model_store="", finetuned=True, no_filtering=False, both_to_prior=False, both_to_context=False, batch_size=32, n_samples=1000, load_in_4bit=False, shots=0, dataset="BaseFakepedia"):
    base_args = [
    ...
    if both_to_prior:
        base_args.append("--both_to_prior")
    if both_to_context:
        base_args.append("--both_to_context")
...
def get_data(args, PATHS, tokenizer, device="cuda"):
    ...
    return target_df, source_df, target_tokens, source_tokens, target_answer_index, source_answer_index, attention_mask_target, attention_mask_source #target_tokens, attention_mask_target, context_1_tokens, context_2_tokens, context_3_tokens, prior_1_tokens, prior_2_tokens, context_1_attention_mask, context_2_attention_mask, context_3_attention_mask, prior_1_attention_mask, prior_2_attention_mask, context_1_answer, context_2_answer, context_3_answer, prior_1_answer, prior_2_answer
...
def get_plot_weightbp_patch(nnmodel, tokenizer, all_tokens, all_attn_mask, context_1_tokens, prior_1_tokens, context_1_attention_mask, prior_1_attention_mask, context_1_answer, prior_1_answer, site_1_config, average_site_config=None, show_patching_flow=False, batch_size=24, N_LAYERS=32, title="", output_dir="plots", api=Llama3):    
    site_1, _, average_site = prepare_sites(site_1_config, {}, average_site_config, api=api, model=nnmodel)
    if average_site_config is not None: 
        average_site = batched_average_cache(nnmodel, all_tokens, all_attn_mask, average_site, batch_size=batch_size)
    else:
    ...
    figr = create_patch_scope_lplot(layers_start_at_one=False, probs=probs, aggregation=aggregation, show_patching_flow=show_patching_flow, ranks=ranks, a_layers={h: site_1_config[h]["layers"] for h in site_1_config}, b_layers={}, avg_layers={h: average_site_config[h]["layers"] for h in average_site_config} if average_site_config is not None else {}, a_title="BOTH", b_title="", c_title="PRI", N_LAYERS=N_LAYERS, title=None, add_rank=True, add_prob=False)
    figr.update_layout(font_family="CMU Serif", width=800)
    figr.write_image(f"{output_dir}/{title}_rank.pdf")
    figp = create_patch_scope_lplot(layers_start_at_one=False, probs=probs, aggregation=aggregation, show_patching_flow=show_patching_flow, ranks=ranks, a_layers={h: site_1_config[h]["layers"] for h in site_1_config}, b_layers={}, avg_layers={h: average_site_config[h]["layers"] for h in average_site_config} if average_site_config is not None else {}, a_title="BOTH", b_title="", c_title="PRI", N_LAYERS=N_LAYERS, title=None, add_rank=False, add_prob=True)
    figp.update_layout(font_family="CMU Serif", width=800)
...
def get_plot_weightbc_patch(nnmodel, tokenizer, all_tokens, all_attn_mask, context_1_tokens, prior_1_tokens, context_1_attention_mask, prior_1_attention_mask, context_1_answer, prior_1_answer, site_1_config, average_site_config=None, show_patching_flow=False, batch_size=24, N_LAYERS=32, title="", output_dir="plots", api=Llama3):    
    ...
    figr = create_patch_scope_lplot(layers_start_at_one=False, probs=probs, aggregation=aggregation, show_patching_flow=show_patching_flow, ranks=ranks, a_layers={h: site_1_config[h]["layers"] for h in site_1_config}, b_layers={}, avg_layers={h: average_site_config[h]["layers"] for h in average_site_config} if average_site_config is not None else {}, a_title="BOTH", b_title="", c_title="CTX", N_LAYERS=N_LAYERS, title=None, add_rank=True, add_prob=False)
    figr.update_layout(font_family="CMU Serif", width=800)
    figr.write_image(f"{output_dir}/{title}_rank.pdf")
    figp = create_patch_scope_lplot(layers_start_at_one=False, probs=probs, aggregation=aggregation, show_patching_flow=show_patching_flow, ranks=ranks, a_layers={h: site_1_config[h]["layers"] for h in site_1_config}, b_layers={}, avg_layers={h: average_site_config[h]["layers"] for h in average_site_config} if average_site_config is not None else {}, a_title="BOTH", b_title="", c_title="CTX", N_LAYERS=N_LAYERS, title=None, add_rank=False, add_prob=True)
    figp.update_layout(font_family="CMU Serif", width=800)
