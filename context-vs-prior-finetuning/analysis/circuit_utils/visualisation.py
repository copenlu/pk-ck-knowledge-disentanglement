...
def get_colors(a_name, b_name):
    ...
    elif "both" in a_name and "ctx" in b_name:
        return COLORS.get_shade(3, 400), COLORS.get_shade(6, 700)
    elif "both" in a_name and "pri" in b_name:
...
def create_patch_scope_lplot(probs, ranks, a_layers, b_layers, avg_layers, show_patching_flow=True, aggregation="median", a_title="Alt CTX", b_title="CTX", c_title="PRIOR", title=None, q_x_offset=0.03, q_y_offset=0.1, N_LAYERS=32, add_rank=True, add_prob=True, layers_start_at_one=True):
  ...
  # increase font size
    ...
    if not show_patching_flow:
        ...
        # add y-axis title
        fig.update_yaxes(title_text=f"Answer Probability", row=1, col=1, title_font=dict(size=18))
