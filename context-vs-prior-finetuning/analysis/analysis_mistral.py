...
def main(args):
    ...
    ## Auto search
    both_to_prior_range = auto_search(model, tokenizer, both_to_prior_args, n_layers=32, phi=0.05, eps=0.3, thres=args.threshold_bp, batch_size=args.batch_size_patch, api=Mistral)
    both_to_context_range = auto_search(model, tokenizer, both_to_context_args, n_layers=32, phi=0.05, eps=0.3, thres=args.threshold_bc, batch_size=args.batch_size_patch, api=Mistral)
    ...
    figr, figp = get_plot_weightbp_patch(nnmodel, tokenizer, *both_to_prior_args, site_1_config, N_LAYERS=32, batch_size=args.batch_size_plot, output_dir=plot_dir, api=Mistral, title=generate_title(site_1_config, f"BP - {model_split_list[-1]}"))
    ...
    figr, figp = get_plot_weightbc_patch(nnmodel, tokenizer, *both_to_context_args, site_1_config, N_LAYERS=32, batch_size=args.batch_size_plot, output_dir=plot_dir, api=Mistral, title=generate_title(site_1_config, f"BC - {model_split_list[-1]}"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--threshold_bc", type=float, default=0.65)
