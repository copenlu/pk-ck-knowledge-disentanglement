...
def compute_positionwise_stats(seqs_rank2, seqs_rank1):
    ...
    for pos in range(max_len):
        ...
        ctx_vals_rank1 = [abs(seq[pos][1]) / (abs(seq[pos][1]) + abs(seq[pos][0])) for seq in seqs_rank1 if len(seq) > pos] 
        mem_vals_rank1 = [abs(seq[pos][0]) / (abs(seq[pos][1]) + abs(seq[pos][0])) for seq in seqs_rank1 if len(seq) > pos]
...
def main(args):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(5.5, 2.5), constrained_layout=False)
    axes = axes.flatten()
    ...
    for idx, dataset in enumerate(dataset_list):
        ...
        all_rank2_path_hal = os.path.join(
            base, f"all_hal_rank2_contrib_{dataset}.pkl"
        )
        all_rank2_path_nonhal = os.path.join(
            base, f"all_nonhal_rank2_contrib_{dataset}.pkl"
        )

        if not os.path.exists(all_rank2_path_hal):
            raise FileNotFoundError(
                f"Missing file: {all_rank2_path_hal}\n"
                "If you intended to plot subsets (supp/comp/etc.), add that logic or create those files."
            )

        all_proj_rank2_hal = load_pickle(all_rank2_path_hal)
        all_proj_rank2_nonhal = load_pickle(all_rank2_path_nonhal)

        # Compute per-position stats
        avg_ctx, std_ctx, avg_mem, std_mem, avg_ctx_r1, std_ctx_r1, avg_mem_r1, std_mem_r1, rank2_len_list, rank1_len_list = compute_positionwise_stats(all_proj_rank2_hal, all_proj_rank2_nonhal)
        ...
        ax.plot(x, avg_ctx_s, label="CK (Hallucinated)")
        ax.fill_between(
            x, avg_ctx_s - std_ctx_s, avg_ctx_s + std_ctx_s, alpha=0.2, linewidth=0
        )

        # Memory line + SD band
        ax.plot(x, avg_mem_s, label="PK (Hallucinated)")
        ax.fill_between(
            x, avg_mem_s - std_mem_s, avg_mem_s + std_mem_s, alpha=0.2, linewidth=0
        )

        ax.plot(x, avg_ctx_s_r1, label="CK (Non-Hallucinated)")
        ax.fill_between(
            x, avg_ctx_s_r1 - std_ctx_s_r1, avg_ctx_s_r1 + std_ctx_s_r1, alpha=0.2, linewidth=0
        )

        ax.plot(x, avg_mem_s_r1, label="PK (Non-Hallucinated)")
        ax.fill_between(
            x, avg_mem_s_r1 - std_mem_s_r1, avg_mem_s_r1 + std_mem_s_r1, alpha=0.2, linewidth=0
        )

        # --- NEW: vertical lines at mean & mode ---
        mean_val_rank2 = np.mean(rank2_len_list)
        mode_val_rank2 = mode(rank2_len_list)

        mean_val_rank1 = np.mean(rank1_len_list)
        mode_val_rank1 = mode(rank1_len_list)

        # Mean line (red, dashed)
        ax.axvline(mean_val_rank2, color="red", linestyle="--", linewidth=1.0,
                   label="Mean length (Hallucinated)")

        # Mode line (blue, dotted)
        if mode_val_rank2 is not None:
            ax.axvline(mode_val_rank2, color="blue", linestyle=":", linewidth=1.0,
                       label="Mode length (Hallucinated)")

        # Mean line (red, dashed)
        ax.axvline(mean_val_rank1, color="darkred", linestyle="--", linewidth=1.0,
                   label="Mean length (Non-Hallucinated)")

        # Mode line (blue, dotted)
        if mode_val_rank1 is not None:
            ax.axvline(mode_val_rank1, color="navy", linestyle=":", linewidth=1.0,
                       label="Mode length (Non-Hallucinated)")
    ...
    fig.legend(
        handles, labels,
        loc="lower center",
        bbox_to_anchor=(0.5, -0.50),  # small negative or positive to fine-tune
        ncol=2,             # put all entries in one row
        frameon=False
    )
    ...
    out_pdf = os.path.join(
        args.output_dir, f"hallucination_rank2_proj_contrib_pk_ck_dynamics_{model_name}_abs.pdf"
    )
