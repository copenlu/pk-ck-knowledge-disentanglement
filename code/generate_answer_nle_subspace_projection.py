...
def main(args):
    ...
    with open(f"{args.data_dir}{args.dataset}/{model_name}/all_input_{args.prompt_mode}_nle.json", 'r') as f:
        all_input = json.load(f)

    proj_rank1 = LowRankOrthogonalProjection.from_pretrained("jkminder/CTXPRIOR-Projection-Meta-Llama-3.1-8B-Instruct-L16").to(args.device)
    proj_rank2 = LowRankOrthogonalProjection.from_pretrained("../context-vs-prior-finetuning/analysis/projections/Meta-Llama-3.1-8B-Instruct-L17").to(args.device)
    ...
    for example_idx, example in enumerate(all_input):
        explanation_tokens_info = example['explanation_tokens']
        try:
            explanation_hs = torch.stack(
                [torch.tensor(seq_step["hidden"], dtype=torch.float32) for seq_step in explanation_tokens_info],
                dim=0
            ).to(args.device)

            explanation_rank1_contrib = explanation_hs @ proj_rank1.weight
            explanation_rank2_contrib = explanation_hs @ proj_rank2.weight
    ...
    with open(f"{args.output_dir}{args.dataset}/{model_name}/all_proj_rank1_{args.prompt_mode}_nle.pkl", 'wb') as f:
        pickle.dump(all_proj_rank1, f)
    with open(f"{args.output_dir}{args.dataset}/{model_name}/all_proj_rank2_{args.prompt_mode}_nle.pkl", 'wb') as f:
        pickle.dump(all_proj_rank2, f)
    with open(f"{args.output_dir}{args.dataset}/{model_name}/supp_proj_rank1_{args.prompt_mode}_nle.pkl", 'wb') as f:
        pickle.dump(supp_proj_rank1, f)
    with open(f"{args.output_dir}{args.dataset}/{model_name}/supp_proj_rank2_{args.prompt_mode}_nle.pkl", 'wb') as f:
        pickle.dump(supp_proj_rank2, f)
    with open(f"{args.output_dir}{args.dataset}/{model_name}/ctx_conf_proj_rank1_{args.prompt_mode}_nle.pkl", 'wb') as f:
        pickle.dump(ctx_conf_proj_rank1, f)
    with open(f"{args.output_dir}{args.dataset}/{model_name}/ctx_conf_proj_rank2_{args.prompt_mode}_nle.pkl", 'wb') as f:
        pickle.dump(ctx_conf_proj_rank2, f)
    with open(f"{args.output_dir}{args.dataset}/{model_name}/prior_conf_proj_rank1_{args.prompt_mode}_nle.pkl", 'wb') as f:
        pickle.dump(prior_conf_proj_rank1, f)
    with open(f"{args.output_dir}{args.dataset}/{model_name}/prior_conf_proj_rank2_{args.prompt_mode}_nle.pkl", 'wb') as f:
        pickle.dump(prior_conf_proj_rank2, f)
    with open(f"{args.output_dir}{args.dataset}/{model_name}/comp_proj_rank1_{args.prompt_mode}_nle.pkl", 'wb') as f:
        pickle.dump(comp_proj_rank1, f)
    with open(f"{args.output_dir}{args.dataset}/{model_name}/comp_proj_rank2_{args.prompt_mode}_nle.pkl", 'wb') as f:
        pickle.dump(comp_proj_rank2, f)
    with open(f"{args.output_dir}{args.dataset}/{model_name}/irr_proj_rank1_{args.prompt_mode}_nle.pkl", 'wb') as f:
        pickle.dump(irr_proj_rank1, f)
    with open(f"{args.output_dir}{args.dataset}/{model_name}/irr_proj_rank2_{args.prompt_mode}_nle.pkl", 'wb') as f:
