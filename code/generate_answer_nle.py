...
def answer_generation(args, model, tokenizer, dataset):
        ...
        for i in tqdm(range(len(dataset))):
            ...
            all_input.append({'context': line['context'], 'query': line['query'], 'label': str(line['label']), 'ctx_label': str(line['ctx_label']), 'prior_label': str(line['prior_label']), 'nle_ans': pred_label, 'explanation': explanation_text, 'explanation_tokens': explanation_tokens_info})
...
def main(args):
    ...
    with open(f"{args.output_dir}{args.dataset}/{model_name}/all_pred_logit_{args.prompt_mode}_nle.pkl", 'wb') as f:
        pickle.dump(all_pred_logit, f)
    with open(f"{args.output_dir}{args.dataset}/{model_name}/all_pred_prob_{args.prompt_mode}_nle.pkl", 'wb') as f:
        pickle.dump(all_pred_prob, f)
    with open(f"{args.output_dir}{args.dataset}/{model_name}/all_pred_hs_{args.prompt_mode}_nle.pkl", 'wb') as f:
        pickle.dump(all_pred_hs, f)
    with open(f"{args.data_dir}{args.dataset}/{model_name}/all_input_{args.prompt_mode}_nle.json", 'w') as f:
