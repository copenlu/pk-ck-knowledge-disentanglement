...
def main(args):
    ...
    for dataset in dataset_list:
        ...
        all_rank2_path = os.path.join(
            base, f"all_proj_rank2_{args.prompt_mode}_nle_answer_token.pkl"
        )
    ...
    ax.legend(
        frameon=False,
        loc="upper center",
        bbox_to_anchor=(0.5, -0.25),  # place below the plot
        ncol=len(dataset_list),       # all datasets in one row
        handlelength=1.5,
        columnspacing=1.0
    )
    ...
    out_pdf = os.path.join(
        args.output_dir, f"rank2_proj_contrib_answer_token_grouped_{model_name}_nle.pdf"
    )
